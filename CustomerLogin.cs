...
namespace Library
{
    public partial class CustomerLogin : Form
    {
        ...
        {
            // Retrieve username/email and password from textboxes
            string input = Username.Text.Trim(); // Can be username or email
            ...
            try
            {
                using (OracleConnection connection = new OracleConnection(connectionString))
                {
                    ...
                    if (input.Contains("@"))
                    {
                        query = "SELECT UserID, UserType, FullName, Username, Password, Email, PhoneNumber, Address, DateCreated FROM Users WHERE Email = :Input AND Password = :Password";
                    }
                    else
                    {
                        query = "SELECT UserID, UserType, FullName, Username, Password, Email, PhoneNumber, Address, DateCreated FROM Users WHERE Username = :Input AND Password = :Password";
                    }
                    ...
                    using (OracleCommand command = new OracleCommand(query, connection))
                    {
                        ...
                        using (OracleDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                {
                                    using (var customerForm = new CustomerForm(userID, fullName, userType, username, Password, email, phoneNo, address, dateCreated))
                                    {
